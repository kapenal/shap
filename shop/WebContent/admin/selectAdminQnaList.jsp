...
<%
	...
	System.out.println(beginRow + "< selectAdminQnaList beginRow");
	// 화면에 보여질 시작 페이지 번호
	int startPage = ((currentPage - 1) / displayPage) * displayPage + 1;
	System.out.println(startPage + "< selectAdminQnaList startPage");
	// 화면에 보여질 마지막 페이지 번호
	int endPage = startPage + displayPage - 1;
	System.out.println(endPage + "< selectAdminQnaList endPage");
	// 전체 회원 수
	QnaDao qnaDao = new QnaDao();
	ArrayList<Qna> qnaList = qnaDao.selectAdminQnaList(beginRow, ROW_PER_PAGE);
	int totalCount = qnaDao.selectQnaListAllByTotalPage();
...
<head>
<meta charset="UTF-8">
<title>selectAdminQnaList.jsp</title>
...
<body>
	<div class="container">
		...
		%>
				<a class="btn btn-info" href="<%=request.getContextPath()%>/selectAdminQnaList.jsp">처음으로</a>
		<%
			}
			// 이전 버튼
		...
		%>
			<a class="btn btn-info" href="<%=request.getContextPath()%>/selectAdmiQnaList.jsp?currentPage=<%=startPage-1%>">이전</a>
		<%
			}
			// 페이지 번호 버튼
			for(int i=startPage; i<=endPage; i++) {
				if(currentPage == i){
		%>
					<a class="btn btn-primary" href="<%=request.getContextPath()%>/selectAdmiQnaList.jsp?currentPage=<%=i%>"><%=i%></a>
		<%	
			} else if(endPage<=lastPage) {
		%>
				<a class="btn btn-info" class="text-warning" href="<%=request.getContextPath()%>/selectAdmiQnaList.jsp?currentPage=<%=i%>"><%=i%></a>
		<%
			} else if(endPage>lastPage) {
		%>
				<a class="btn btn-info" href="<%=request.getContextPath()%>/selectAdmiQnaList.jsp?currentPage=<%=i%>"><%=i%></a>
		<%	
			}
			...
			if(endPage < lastPage) {
		%>
			<a class="btn btn-info" href="<%=request.getContextPath()%>/selectAdmiQnaList.jsp?currentPage=<%=startPage+displayPage%>">다음</a>
		<%
			}
			// totalCount가 10보다 크면 다음페이지가 있기때문에 끝으로 보이도록 설정
			if(totalCount > ROW_PER_PAGE && currentPage != lastPage ) {
		%>
				<a class="btn btn-info" href="<%=request.getContextPath()%>selectAdmiQnaList.jsp?currentPage=<%=lastPage%>">끝으로</a>
		<%
			}
		%>
	</div>
